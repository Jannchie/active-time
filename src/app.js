...
import EchartsComp from "./echarts-wrapper";
function App() {
  ...
  useEffect(() => {
    async function setInitData() {
      const { systemInfo } = await window.electronAPI.ready();
      setSystemInfo(systemInfo);
    }
    window.electronAPI.toggleRecord(true);
    window.electronAPI.onRecordingChanged((sender, val) => {
      setRecording(val);
    });
    window.electronAPI.onActiveEvent((sender, { event, activeWindowsInfo }) => {
      setActiveWindows(activeWindowsInfo);
    });
    setInitData();
    return () => {
  ...
  return (
    <div className="h-screen w-screen dark:text-zinc-300 dark:bg-zinc-900 dark:fill-slate-100">
      ...
      <div>
        ...
        {/* <div>OS: {activeWindows.os}</div>
        <div>windowClass: {activeWindows.windowClass}</div>
        <div>windowName: {activeWindows.windowName}</div>
      ...
      <div className="sticky dark:bg-zinc-800 flex" style={{ top: "100vh" }}>
        <div
          ...
          onClick={() => {
            window.electronAPI.toggleRecord(!recording);
