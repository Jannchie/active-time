      ...
      <button
        ...
        className={`${
          type === 'danger'
            ...
            : 'bg-zinc-500/20 hover:bg-zinc-500/40 border border-zinc-500/40'
        } rounded-md transition cursor-pointer bg-opacity-20 px-4 py-2 text-sm font-medium text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}
      >
        {title}
      </button>
    ...
    <div className="flex flex-col mx-4 my-2 gap-2">
      <div>
        <div className="p-2 border rounded-lg border-zinc-500/20 mb-2">
          ...
          <button
            type="button"
            className={`${!active('dark') || 'j-btn-active'} j-btn`}
            onClick={() => {
              setThemeValue('dark');
          ...
          <button
            type="button"
            className={`${!active('light') || 'j-btn-active'} j-btn`}
            onClick={() => {
              setThemeValue('light');
          ...
          <button
            type="button"
            className={`${!active('system') || 'j-btn-active'} j-btn`}
            onClick={() => {
...
function App() {
  // const ctx = createContext({ theme: 'system' });
  ...
  const page = ((p: number) => {
    switch (p) {
      case 0:
        return RecentPage({ minutesRecordsResp });
      case 1:
        return <SettingsPage />;
      default:
        return RecentPage({ minutesRecordsResp });
  ...
  return (
    <div className="flex flex-col h-screen w-screen dark:text-zinc-300 dark:bg-zinc-900 bg-zinc-100 dark:fill-slate-100 overflow-clip">
      <div
        className="flex justify-between items-center dark:bg-zinc-800 z-10 bg-zinc-100 w-full"
      ...
      <div className="flex overflow-hidden flex-grow">
        ...
        <div className="flex-grow overflow-x-hidden">{page}</div>
      </div>
